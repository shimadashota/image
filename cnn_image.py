...
with open('../data/out/data_z.pickle','rb') as f:
    train_imgs_model = pickle.load(f)
    train_label_model = pickle.load(f)
...
test_imgs_model = test_imgs_model.reshape(-1,num_input)
...
with tf.Session() as sess:
    ...
    for step in range(1, num_steps+1):
        ...
        if step % display_step == 0 or step == 1:
            ...
            loss, acc = sess.run([loss_op, accuracy], feed_dict={X: train_imgs_model,
                                                                 Y: train_label_model,
                                                                 keep_prob: 1.0})
    ...
    print("Testing Accuracy:", \
        sess.run(accuracy, feed_dict={X: test_imgs_model,
                                      Y: test_label_model,
                                      keep_prob: 1.0}))
